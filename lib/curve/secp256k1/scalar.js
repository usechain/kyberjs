...
class Scalar extends group.Scalar {
   ...
   */
  setBytes(b) {
    if(b==undefined) {throw new TypeError("b is empty");}
  ...
  unmarshalBinary(bytes) {
    ...
    if (bytes.length !== this.marshalSize()) {
      console.log(`bytes.length:",${bytes.length},bytes:${bytes.toString("hex")}`)
      console.log("this.marshalSize():",this.marshalSize())
